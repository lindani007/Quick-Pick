...
namespace QuickPick_Employer.QuickPickEmployer.ViewModel
{
    public partial class TodayTransactionViewModel : ObservableObject
    {
        ...
        TransactionListViewModel _transactionListViewModel;
        public TodayTransactionViewModel(SlesService slesService, DbBoughtItemService dbBoughtItemService
            , ItemService itemService, OrderService orderervice, TransactionListViewModel transactionListViewModel)
        {
        ...
        private async void LoadOrders()
        {
            ...
            if (List.Count > 0)
            {
                List = List.Where(d => d.OrderDate.Date == DateTime.Today).ToList();
        ...
        public async Task GetSelectedOrder()
        {
            ...
            {
              _transactionListViewModel.OrderedItems = new ObservableCollection<BoughtItem>(await ReturnBoughtItemsForOrder(SelectedOrder));
                await App.Current.MainPage.Navigation.PushAsync(new Ordered_Items(_transactionListViewModel));
                SelectedOrder = null;
            }
        }
        [RelayCommand]
        public async Task ViewItemTransaction(BoughtItem item)
        {
            var l = await ReturnBoughtItemsByNames();
            l = l.OrderByDescending(d => d.TransactionDtae).ToList();
            List<BoughtItem> list = l.Where(i => i.ItemId == item.ItemId && i.TransactionDtae.Date == DateTime.Today).ToList();
            await App.Current.MainPage.Navigation.PushAsync(new DetailedTRansaction(list));
        }
        [RelayCommand]
        private async Task RefundItems(BoughtItem item)
        {
            ...
            await App.Current.MainPage.Navigation.PushAsync(new Refund_Item(_transactionListViewModel));
        }
        [RelayCommand]
        ...
        private async Task<List<BoughtItem>> ReturnBoughtItemsByNames()
        {
            ...
            itemsBought =  itemsBought.Where(x => x.TransactionDtae.Date ==  DateTime.Today).ToList();
            return itemsBought;
        }
        private async Task<List<BoughtItem>> ReturnBoughtItemsForOrder(Order order)
        {
            List<BoughtItem> items = await GetItems();
            items = items.Where(x => x.OrderId == order.OrderId && x.TransactionDtae.Date == DateTime.Today).ToList();
